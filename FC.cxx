...
{
  if (argc!=3) {
    std::cerr<<"usage: "<<argv[0]<<" <using nomal modes: 0=yes; 1=No (using Force constants instead)> <input filename>"<<std::endl;
    exit(0);
  }
...
//   Lmation=RotateCartesianMW*Lmation;
  ...
  if (usingnormalmodes != 0) {
    ...
    vibreportoutname<<filenamein.str()<<".vibanalysis.stat"<<ends;
    ofstream vibreportout(vibreportoutname.str());

    vibreportout<<"mode";
    vibreportout<<" initial state (1/CM)";
    vibreportout<<" final state (1/CM)"<<std::endl;
    for (k=0;k<(3*natom);k++) 
      vibreportout<<k<<" "<<sqrt(complex(NMn(k),0.))*hatocm<<" "<<sqrt(complex(NMc(k),0.))*hatocm<<std::endl;    
    // inertia tensor
    matrix In(3,3);
    for (i=0;i<natom;i++) {
...
//   }
  ...
  Smatrixname <<filenamein.str()<<".S.mat"<<ends;
  ofstream Smatrix(Smatrixname.str());
  std::strstream OrSname;
  OrSname<<filenamein.str()<<".OrS.mat"<<ends;
  ofstream OrS(OrSname.str());
  for (i=0;i<(3*natom-6);i++) {
    for (j=0;j<(3*natom-6);j++) {
      Smatrix.precision(2);
      Smatrix.width(8);
  ...
  dvecfilename<<filenamein.str()<<".d.vec"<<ends;
  ofstream dvecfile(dvecfilename.str());
  for (i=0;i<(3*natom-6);i++) {
  ...
  statname<<filenamein.str()<<".spectra.stat"<<ends;
  ofstream stat(statname.str());
